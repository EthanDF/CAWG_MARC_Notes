...
def buildKeyBibNotesDB():
    ...
    c.execute('select distinct b.keybibNotes, c.OCLC, b.NoteOrder, b.note from alephBibs a join bibNotes b on a.bibNumber = b.bib join altOCLC c on a.oclc = c.altoclc join oclcNotes o on c.oclc = o.oclc left join notesAnalysis n on b.keybibNotes = n.KeybibNotes where a.oclc > 0 and a.GPO = 0 and a.gPub = 0 and b.OwnCodeCount = 1 and n.KeybibNotes is null')
    all_rows = c.fetchall()
...
def getBibToOCN():
    ...
    c.execute('SELECT distinct a.bibNumber, c.oclc FROM alephBibs a join altOCLC c on a.oclc = c.oclc where a.oclc > 0 and a.GPO = 0 and a.gPub = 0')
